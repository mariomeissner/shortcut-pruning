"""
This was taken from: 
https://github.com/NicholasYuan/bert_delta/blob/master/BERT-SQuAD/eval_squad.py
    ...
    for article in dataset:
        for paragraph in article["paragraphs"]:
            for qa in paragraph["qas"]:
                ...
                if orig_id != qa["id"]:
                    continue  # Skip the mutated ones
                ...
                print("Answers: [%s]" % ", ".join(a["text"].encode("utf-8") for a in qa["answers"]))
                orig_color = get_answer_color(predictions[orig_id], qa["answers"])
                print("Predicted: %s" % colored(predictions[orig_id], orig_color).encode("utf-8"))
                print("Adversary succeeded?: %s" % (adv_id != orig_id))
                if adv_id != orig_id:
                    print(
                        "Adversarial Paragraph: %s"
                        % highlight_after(id_to_paragraph[adv_id], len(paragraph["context"])).encode("utf-8")
                    )
                    ...
                    print("Prediction under Adversary: %s" % colored(predictions[adv_id], adv_color).encode("utf-8"))
                print("")
...
def evaluate_adversarial(dataset, predictions, verbose=False, id_set=None):
    ...
    for article in dataset:
        for paragraph in article["paragraphs"]:
            for qa in paragraph["qas"]:
                ...
                if id_set and orig_id not in id_set:
                    continue
                all_ids.add(orig_id)
                ...
                cur_exact_match = metric_max_over_ground_truths(exact_match_score, prediction, ground_truths)
                ...
                if orig_id == qa["id"]:
                    ...
                    if orig_id not in adv_f1_scores or adv_ids[orig_id] == orig_id or adv_f1_scores[orig_id] > cur_f1:
    ...
    orig_f1 = orig_f1_score / len(all_ids)
    orig_exact_match = orig_exact_match_score / len(all_ids)
    adv_exact_match = sum(adv_exact_match_scores.values()) / len(all_ids)
    adv_f1 = sum(adv_f1_scores.values()) / len(all_ids)
    return OrderedDict(
        [
            ("oe", orig_exact_match),
            ("of", orig_f1),
...
def evaluate_by_attempted(dataset, predictions):
    ...
    results_attempted = evaluate_adversarial(dataset, predictions, id_set=attempted)
    print("Attempted %d/%d = %.2f%%" % (len(attempted), total_num, 100.0 * len(attempted) / total_num))
    print(json.dumps(results_attempted))
    results_not_attempted = evaluate_adversarial(dataset, predictions, id_set=not_attempted)
    print("Did not attempt %d/%d = %.2f%%" % (len(not_attempted), total_num, 100.0 * len(not_attempted) / total_num))
    print(json.dumps(results_not_attempted))


if __name__ == "__main__":
    expected_version = "1.1"
    parser = argparse.ArgumentParser(description="Adverarial evaluation for SQuAD " + expected_version)
    parser.add_argument("dataset_file", help="Dataset file")
    parser.add_argument("prediction_file", help="Prediction File")
    parser.add_argument("--out-file", "-o", default=None, help="Write JSON output to this file (default is stdout).")
